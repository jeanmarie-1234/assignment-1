isolated service class StaffRepository {
    ...
    private table<Course> key(courses_codee) courses = table [
        {courses_codee: "PLU", courses: "Principles of english language use", nqf_level: 7},
        {courses_codee: "PRG", courses: "Programming", nqf_level: 7},
        {courses_codee: "OPS", courses: "Operating Systems", nqf_level: 7},
        {courses_codee: "DPG", courses: "Database Programming", nqf_level: 7}
    ];
    ...
    isolated function getLecturersByCourseCode(string courses_codee) returns Lecturer[]|error{
        lock{
            return (from Lecturer l in self.lecturers.clone()
                where l.list_of_courses.indexOf(courses_codee) != ()
