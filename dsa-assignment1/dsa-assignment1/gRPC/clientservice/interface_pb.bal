...
public isolated client class LibraryClient {
    *grpc:AbstractClientEndpoint;
    ...
    isolated remote function add_book(Book|context_book req) returns ISBN|grpc:Error {
        map<string|string[]> headers = {};
        Book message;
        if req is context_book {
            message = req.content;
            headers = req.headers;
        } else {
            message = req;
        }
        var payload = check self.grpcClient->executeSimpleRPC("Library/add_book", message, headers);
        [anydata, map<string|string[]>] [result, _] = payload;
        return <ISBN>result;
    }

    isolated remote function addBookContext(Book|context_book req) returns ContextISBN|grpc:Error {
        map<string|string[]> headers = {};
        Book message;
        if req is context_book {
            message = req.content;
    ...
    isolated remote function updateBook(Book|context_book req) returns Message|grpc:Error {
        map<string|string[]> headers = {};
        Book message;
        if req is context_book {
            message = req.content;
    ...
    isolated remote function updateBookContext(Book|context_book req) returns ContextMessage|grpc:Error {
        map<string|string[]> headers = {};
        Book message;
        if req is context_book {
            message = req.content;
...
public type context_book record {|
    Book content;
    map<string|string[]> headers;
|};
